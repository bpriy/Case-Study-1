...
# The Basic Two-Step Phenology Model (With September 1st As Dormancy Initiation Date and Parameters)
TC = 5 # threshold temperature below which chill days (Cd) accumulate
Rc = -69 # chill requirement
Rh = 210.5 # heat requirement
ppd = 0 # predicted peak bloom date 
year = 0 
chill_days = 0
heat_days = 0
for (i in 1:y){ # for each year from 1954:2022
  #### Dormancy Period
  # calculate daily "chill day" values starting from September 1st
  ...
  chill <- min(which(tab$cm_cd <= Rc))  # number of days until dormancy released = number of days to first reach at least the Rc value (since September 1st)
  if (!is.finite(chill)) { 
    ppd[i] = "NA"
    chill_days[i] <- "NA"
    heat_days[i] <- "NA"
  ...
  totdays <- min(which(tab$cm_ca >= Rh)) # total number of days to reach peak bloom (since September 1st)
  ...
  ppd[i] = totdays - 122 # convert the totdays value to "day of year" (starting from January 1st) & store the "ppd" value obtained in each for-loop iteration (year)
...
fit = lm(lrestab$ppd ~ restab$bloom_doy) 
summary(fit)
