...
# The Basic Two-Step Phenology Model (With November 1st As Dormancy Initiation Date and Parameters)
TC = 5 # threshold temperature below which chill days (Cd) accumulate
Rc = -135.1 # chill requirement
Rh = 264.6 # heat requirement
...
for (i in 1:y){ # for each year from 1945 to 2021
  #### Dormancy Period
  # calculate daily "chill day" values starting from November 1st
  ...
  chill <- min(which(tab$cm_cd <= Rc))  # number of days until dormancy released = number of days to first reach at least the Rc value (since November 1st)
  if (!is.finite(chill)) { 
    ppd[i] = "NA"
    year[i] = datal[[i]][1,5]
    next
  ...
  totdays <- min(which(tab$cm_ca >= Rh)) # total number of days to reach peak bloom (since November 1st)
  ...
  ppd[i] = totdays - 62 # convert the totdays value to "day of year" (starting from January 1st) & store the "ppd" value obtained in each for-loop iteration (year)
